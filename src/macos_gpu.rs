...
impl AppleGpuSampler {
    pub fn new() -> Option<Self> {
        unsafe {
            ...
            let energy_channels = IOReportCopyChannelsInGroup(energy_group, ptr::null(), 0, 0, 0);
    ...
    pub fn sample(&mut self) -> Option<AppleGpuMetrics> {
        unsafe {
            let current = IOReportCreateSamples(self.subscription, self.channels, ptr::null());
            if current.is_null() {
                return None;
            }
    ...
    unsafe fn parse_delta(&self, delta: CFDictionaryRef) -> AppleGpuMetrics {
        ...
        for i in 0..count {
            ...
            let group = cfstring_to_string(IOReportChannelGetGroup(channel)).unwrap_or_default();
            let subgroup =
                cfstring_to_string(IOReportChannelGetSubGroup(channel)).unwrap_or_default();
            let name =
                cfstring_to_string(IOReportChannelGetChannelName(channel)).unwrap_or_default();
